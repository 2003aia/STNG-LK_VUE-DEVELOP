    ...
    actions: {
        ...
        getAgreements: async ({ commit, dispatch, state }) => {
            ...
            if (state.user.isLoggedIn) {
                ...
                const fetchAgreements = await fetch(`https://1c.aostng.ru/VESTA/hs/API_STNG_JUR/info?token=${state.user.token}`, {
                    mode: "cors",
                    method: "get",
                    // body: JSON.stringify({
                    ...
                    if (fetchAgreements.ok) {
                        ...
                        if (router.currentRoute.params.id) {
                            ...
                            const currentAgreement = jsonAgreements.data.find(
                                item =>
                                    item.id === router.currentRoute.params.id
                            );
                            console.log(
        ...
        authUser: async (ctx, userObject) => {
            if (Vue.cookie.get("token")) {
                ...
                const res = await fetch('https://1c.aostng.ru/VESTA/hs/API_STNG_JUR/V1/login', {
                    mode: "cors",
                    method: "post",
                    body: JSON.stringify({
        ...
        forgotUser: async (ctx, userObject) => {
            ...
            const res = await fetch('https://1c.aostng.ru/VESTA/hs/API_STNG_JUR/V1/jur_restore', {
                mode: "cors",
                method: "post",
                body: JSON.stringify({
        ...
        init: async ctx => {
            ...
            if (Vue.cookie.get("token")) {
                ...
                console.log(
                    "Токен отсутствует в куки, требуется авторизация..."
                );
            }
        },
