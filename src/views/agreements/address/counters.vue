...
.agreements-address-counter
    ...
    .history__table
        ...
        .history__table-row(v-for="item in records" :key="item.id")
            ...
            .history__table-cell
                input(type="number" v-model="item.value" placeholder="Введите показания счетчика")
    .mobile-table.mobile-table_variant(v-if="isMobile")
        .mobile-table__item(v-for="item in records" :key="item.id")
            ...
            .mobile-table__field
                ...
                .mobile-table__value
                    input(type="number" v-model="item.value" placeholder="Введите показания")
    .notice(v-if="notice")
        p {{ notice }}
    ...
    methods: {
        ...
        sendIndication () {
            ...
            for (let record of this.records) {
                ...
                if (record.value !== '') {
                    if (+record.value < record.old_indication) return this.notice = `Показания по счетчику ${record.counterId} меньше предыдущих показаний`;
                    if ((record.value.length - old_indication_str) > 2) return this.notice = `Показания по счетчику ${record.counterId} слишком большая разница между текущими и предыдущими показаниями`;
